...
диапазон от следующего slot за исходной границей prefill до последней закрытой
candle; `LIVE` публикуется только после проверки обоих диапазонов. Обычный
`PREFILL` остаётся startup-only: он повторяет исходный диапазон и не добавляет
recovery хвоста outage. После его завершения последующие transport statuses не
включают recovery неявно.
