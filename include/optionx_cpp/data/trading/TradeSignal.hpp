...
namespace optionx {
    ...
    public:
        ...
        std::string assumed_reason;                         ///< Machine-readable reason for an assumed signal.
        ...
        void assign_to_request(TradeRequest& request) const {
            ...
            request.is_assumed = is_assumed;
            request.assumed_reason = assumed_reason;
            request.assumed_source_identity = assumed_source_identity;
            request.assumed_source_step = assumed_source_step;
        ...
        std::unique_ptr<TradeSignal> clone() const {
            ...
            cloned->is_assumed = is_assumed;
            cloned->assumed_reason = assumed_reason;
            cloned->assumed_source_identity = assumed_source_identity;
            cloned->assumed_source_step = assumed_source_step;
    ...
    struct adl_serializer<optionx::TradeSignal> {
        ...
        static void to_json(json& j, const optionx::TradeSignal& signal) {
            j = json{
                ...
                {"is_assumed", signal.is_assumed},
                {"assumed_reason", signal.assumed_reason},
                {"assumed_source_identity", signal.assumed_source_identity},
                {"assumed_source_step", signal.assumed_source_step},
        ...
        static void from_json(const json& j, optionx::TradeSignal& signal) {
            ...
            signal.is_assumed = j.value("is_assumed", false);
            signal.assumed_reason = j.value("assumed_reason", std::string());
            signal.assumed_source_identity = j.value(
                "assumed_source_identity", std::string());
            signal.assumed_source_step = j.value("assumed_source_step", -1);
